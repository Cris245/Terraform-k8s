...
variable "primary_region" {
  ...
  type        = string
  default     = "us-central1"  # Iowa - Global primary
}

variable "secondary_region" {
  description = "Secondary GCP region for failover"
  type        = string
  default     = "europe-west1"  # Belgium - European secondary
}
...
variable "node_pools" {
  ...
  default = {
    ...
    secondary = {
      machine_type = "e2-standard-8"  # 8 vCPUs, 32 GB RAM - Same power for true HA
      node_count   = 3                # Same node count for consistency
      disk_size_gb = 80
      disk_type    = "pd-ssd"
      preemptible  = true             # Keep preemptible for cost savings
    }
  }
}
...
variable "audit_dataset_location" {
  ...
  type        = string
  default     = "US"  # Aligns with US primary deployment
}
